...
func BuildDcRequestBaiduPc(rbr *request_builder.RequestBuilderRequest) *request_builder.DcRequest {
	...
	if rbr.Page == 1 {
		requestUrl = fmt.Sprintf("https://www.baidu.com/s?wd=%s", url.QueryEscape(rbr.SearchWord))
	} else {
		requestUrl = fmt.Sprintf("https://www.baidu.com/s?wd=%s&pn=%d", url.QueryEscape(rbr.SearchWord), (rbr.Page-1)*10)
	}
	...
	dcRequest.Request.UserAgent = utils.RandomUserAgentForEngine("baidu_pc")
