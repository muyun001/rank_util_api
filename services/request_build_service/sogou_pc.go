...
func BuildDcRequestSogouPc(rbr *request_builder.RequestBuilderRequest) *request_builder.DcRequest {
	...
	if rbr.Page == 1 {
		requestUrl = fmt.Sprintf("https://www.sogou.com/web?query=%s&ie=utf8&s_from=index&cid=", url.QueryEscape(rbr.SearchWord))
	} else {
		requestUrl = fmt.Sprintf("https://www.sogou.com/web?query=%s&ie=utf8&s_from=index&cid=&page=%d", url.QueryEscape(rbr.SearchWord), rbr.Page)
	}
